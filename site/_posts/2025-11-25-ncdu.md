...
```bash
sudo chmod a+r /etc/ncdu.conf
```

## Safe configuration for `ncdu` version < 1.19 (tested on [20.04](/tag/20.04.html){:.set-5} and [22.04](/tag/22.04.html){:.set-5})
:warning: This method **does not work with sudo**, and does not work with non-interactive shells. This problem should be fixed, but probably won't.
   ...
   * ***OR*** run this script:
     ...
     ```
   * for `zsh`, add `alias ncdu='ncdu -rr` in ``
