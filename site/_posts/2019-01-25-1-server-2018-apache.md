...
1. Install `certbot`:
    ```
    sudo apt-get install -y python3-certbot
    ```
1. Configure `certbot`, this will request and install certificates, `cron` will
   ...
   must be able to access it.
     ```
     sudo sudo certbot --standalone certonly -d foo.example.org -d bar.example.com -d baz.example.com
...
1. Edit /etc/apache/site-available/bar-ssl.conf:
    ...
    <IfModule mod_ssl.c>
    	<VirtualHost _default_:443>
            ...
            # Proxy for the web sockets, notice the ws:// protocol
            ProxyPass         /hts/comet/ws ws://localhost:9981/hts/comet/ws
            ProxyPassReverse  /hts/comet/ws  ws://localhost:9981/hts/comet/ws

            # Proxy for the web interface
            ProxyPass         /hts  http://localhost:9981/hts
            ProxyPassReverse  /hts  http://localhost:9981/hts
